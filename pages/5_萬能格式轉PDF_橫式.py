...
# 網頁基礎配置與動態環境偵測引擎
# ==========================================
st.set_page_config(page_title="萬能格式轉 PDF (橫式)", layout="wide")
...
def convert_excel(input_path, output_path, log):
    ...
    try:
        ...
        wb.Close(False)
        log.text(f"✅ Excel 轉換成功 (橫式自動分頁): {os.path.basename(input_path)}")
    except Exception as e:
...
# 網頁介面渲染區
# ==========================================
st.title("📄 萬能轉 PDF (橫式單頁寬)")
st.write("支援 Word、Excel 與圖片轉為 PDF。**內建橫式底稿引擎**：Excel 自動換行、鎖定標題列，橫向寬度縮放一頁，高度自動分頁。")
...
if IS_WINDOWS:
    ...
    if files and st.button("🚀 開始橫式轉換", type="primary"):
        task_id = time.strftime("%Y%m%d_%H%M%S")
        WORK_DIR = os.path.abspath(f"temp_conv_h_{task_id}")
        OUT_DIR = os.path.abspath(f"out_conv_h_{task_id}")
        ...
        st.success("🎉 橫式轉換完畢！")
        zip_name = f"PDF_Landscape_{task_id}"
        shutil.make_archive(zip_name, 'zip', OUT_DIR)
        
        with open(f"{zip_name}.zip", "rb") as z:
            st.download_button("📥 下載橫式 PDF 結果 (ZIP)", z, file_name="橫式轉換結果.zip", type="primary")
    ...
    為嚴格確保查核底稿轉換後的橫向排版不走位、表格欄寬完整縮放至一頁寬以符合原廠底稿規範，本重裝轉檔模組需直接調用微軟官方 Windows 原生 COM 轉譯引擎。
    
    👉 **如何解鎖本功能？**
    請直接於您個人辦公室電腦本機端運行本專案（執行 `streamlit run app.py`），系統將自動偵測並 100% 完整解鎖橫向排版轉檔介面！
    """)
    
    # 放置視覺佔位符維持版面專業度
    st.file_uploader("🔒 本機專屬模組 (雲端模式暫時鎖定)", disabled=True)
    st.button("🚀 開始橫式轉換", disabled=True, type="primary")